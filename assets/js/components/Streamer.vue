<template>
  <div class="container text-center mx-auto">
    <div class="container flex-col">
      <div>
        <h2 class="cursor-default b-6 text-4xl font-bold">
          {{ info.display_name }}
        </h2>
      </div>
      <div class="flex flex-row align-items-center justify-center space-x-4">
        <div v-bind:style="artBg" class="w-auto p-4">
          <a
            :href="'https://www.twitch.tv/' + info.display_name"
            target="_twitch"
          >
            ...
            <twitch-art
              v-else-if="info.profile_image_url"
              :imageType="'profile'"
            ...
            <div>
              <span
                v-if="broadcast"
                class="cursor-default text-gray-800 bg-red-400 p-1 rounded-sm"
              >
                Live
              </span>
              <span
                v-else="broadcast"
                class="cursor-default text-gray-800 bg-gray-400 p-1 rounded-sm"
              >
                Offline
              </span>
              <span class="cursor-default font-xl p-2">
        ...
        <div v-bind:style="embedBg" class="w-auto p-4">
          <twitch-embed
            v-if="info.login"
            v-bind:embedData="{
              elementId: 'twitch-embed-' + info.login,
              channel: info.login,
            }"
          ></twitch-embed>
          <div
          ...
          >
            ...
            <div class="pl-4">
              <img class="inline" src="/assets/images/red-eye.png" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="displayTab === 'recent'">
        ...
        <div class="flex flex-row flex-grow flex-wrap justify-left p-8">
          <div v-for="vod in vods" class="rounded-md m-8 p-4">
            <twitch-embed
              v-bind:embedData="{
                elementId: 'twitch-embed-vod-' + vod.id,
                video: vod.id,
              }"
            ></twitch-embed>
          </div>
    ...
    <div v-if="displayTab === 'about'">
      <div class="pt-6 font-bold text-xl">
        <p class="cursor-default rounded-md text-left text-xl">
          {{
            info.description
              ? info.description
              : "This streamer hasn't added a description"
          }}
        </p>
      </div>
    </div>
...
export default {
  ...
  mounted: function () {
    ...
    axios.get(dataUrl).then((response) => {
      ...
      this.broadcast =
        response.data.broadcast.length > 0 ? response.data.broadcast[0] : null;
